...
psych_summary <- psych_wellbeing %>% 
  ...
  summarise(
    ...
    sd = sd(fs_score),
    n = n(),
    se = sd / sqrt(n), # remove this for ariginal, and add it to the improved graph
    .groups = "drop"
  ) %>% 
  mutate(
...
ggplot(psych_summary, aes(x = time, y = mean, group = intervention_group)) +
  ...
  geom_text(aes(label = paste0("M = ", round(mean, 2)),
                hjust = ifelse(intervention_group == "HIIT - High Intensity Interval Training Group", -0.15, 1.15)),
            vjust = -0.5, size = 3.2) +
  scale_linetype_manual(
    values = c(
      "HIIT - High Intensity Interval Training Group" = "solid",
      "EDU - Education" = "dashed")) +
  labs(
...
# Enhanced graph + add position dodge
ggplot(psych_summary, aes(x = time, y = mean, group = intervention_group)) +
  geom_line(aes(colour = intervention_group), linewidth = 0.8) +
