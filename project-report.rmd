...
The dataset provided contains missing metadata: while the authors of the study are clear, they failed to identify exactly who collected and analysed the data, thereby decreasing the findability of the dataset. However, other aspects of the metadata are clear: the year of data collection is given, and the title of the dataset is clear and concise. Additionally, it is clear that the dataset is an Excel document, there is a persistent identifier in the form of a DOI and contact details for one of the researchers are given. These factors all influence the findability of the data. However, while the naming of the folder for the dataset is clear, the naming conventions of columns in the dataset itself were variable, for example, they switch between using ‘fs’ and ‘flourishing scale’. For lay audiences, these acronyms can be hard to decipher. This issue is compounded by the lack of explanation of these acronyms in the supporting information. While definitions of the acronyms can be found in the tables included in the original article, explanations of acronyms used should be included in the metadata to make the dataset FAIRer by increasing interoperability. Additionally, metadata regarding what ‘1’ and ‘2’ means in terms of intervention groups is missing. Correcting this would result in greater interoperability. The dataset is available under a Creative Commons International licence, allowing anyone to download the dataset and modify it. The data includes no identifiable information, so doesn’t require authentication or authorisation. This increases its accessibility and reusability. The dataset is available on Mendeley Data and archived in the Data Archiving and Network Services (DANS) where it is stored ‘in perpetuity’ (Mendeley Data, n.d.), meaning that the metadata of the dataset will be available in the DANS even if the original dataset is deleted, increasing its accessibility.
...
Mendeley Data (n.d.). *Dataset Archiving.* Available at: <https://data.mendeley.com/archive-process> (Accessed: 31 Jan. 2026).
