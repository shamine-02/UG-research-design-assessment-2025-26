...
fig3_postpartum_depression <- postpartum_depression %>% 
  ...
  rename("edss" = edinburg_postnatal_depression_scale_score) %>% 
  drop_na()
  
  
levels <- fig3_postpartum_depression %>% 
  mutate(depression_level = case_when(edss <10 ~ "norm",
                           edss >=10 & edss<=11 ~ "slightly increased",
                           edss>11 ~ "increased")) %>% 
  group_by(depression_level, intervention_group) %>% 
  count(depression_level) %>% 
  pivot_wider(id_cols = depression_level, names_from = intervention_group, values_from = "n")
```
